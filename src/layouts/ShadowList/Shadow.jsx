...
export default function Shadow({panelNumber, shadow}) {
    ...
    const shadowInputs = shadow.inputs.map((input, index) => {
        if(input.type === "range") return <ShadowRange key={index} inputData={shadow.inputs.index} shadowID={shadow.id}/> 
        else if (input.type === "color") return <ShadowColorPicker key={index} inputData={shadow.inputs.index} shadowID={shadow.id}/>
      })
  return (
